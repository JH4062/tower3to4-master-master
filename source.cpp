...
ObjectID messageObj, defeatMessage;
...
ObjectID CanNotRestMessage, casinoMessage;
...
// Location of zomhand
int zomhandX[5] = { 340, 540, 640, 740, 940 };
int zomhandY = 500;
...
void gameInit() {
	...
	CanNotRestMessage = createObject("./Images/Messages/CanNotRest.png");
	locateObject(CanNotRestMessage, villageLeftScene, 0, 0);

	defeatMessage = createObject("./Images/Messages/DefeatMessage.png");
	locateObject(defeatMessage, villageLeftScene, 0, 0);

	casinoMessage = createObject("./Images/Messages/CasinoMessage.png");
	locateObject(casinoMessage, casinoScene, 190, -150);
	scaleObject(casinoMessage, 0.7f);
...
void checkHp(int kind) {
	...
	if (0.75f < hpPercent and hpPercent <= 1.0f) {
		...
		//BugList :: 1 =======================================================
		// when player dies,
		// 1. rid of all of player's gold
		gold = 0;

		// 2. starts at villageLeftScene
		currentScene = villageLeftScene;
		locateObject(player, currentScene, 100, playerY_FIXED);
...
void zombieAtt0() {
	...
	if (checkCollision(playerIcon, zombieX[repeatNum], (zombieX[repeatNum] + 120), zombieY, zombieY + 213) && hitAlready == false) {
...
void mouseCallback(ObjectID object, int x, int y, MouseAction action) {
	...
	if (object == defeatMessage) {
		hideObject(defeatMessage);
	}
	// =======================================================================

	if (object == casinoMessage) {
		hideObject(casinoMessage);
	}
...
void keyboardCallback(KeyCode code, KeyState state) {
	...
	if (code == 83) {				// RIGHT
		...
		else if (enemyShown == true && currentScene == tower4F_Scene && playerX >= 750) {
